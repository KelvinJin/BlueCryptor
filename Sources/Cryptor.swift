...
public class Cryptor : StreamCryptor, Updateable {
	...
	public func update(from buffer: UnsafeRawPointer, byteCount: Int) -> Self? {
        ...
        var dataOutMoved = 0
        _ = update(bufferIn: buffer.bindMemory(to: UInt8.self, capacity: byteCount), byteCountIn: byteCount, bufferOut: &dataOut, byteCapacityOut: dataOut.count, byteCountOut: &dataOutMoved)
		if self.status != .success {
			return nil
		}
        accumulator += dataOut[0..<Int(dataOutMoved)]
